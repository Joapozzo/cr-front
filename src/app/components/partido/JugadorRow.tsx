...
interface JugadorRowProps {
    ...
    haySuplentesDisponibles?: boolean;
...
const JugadorRow: React.FC<JugadorRowProps> = ({
    ...
    const contenidoRow = (
        <div
            ...
            onClick={(e) => {
                // Solo abrir modal si no se clickeó un botón o icono dentro de un botón
                const target = e.target as HTMLElement;
                if (target.closest('button') || target.tagName === 'BUTTON' || target.closest('svg')?.closest('button')) {
                    return;
                }
                // Solo abrir modal en modo planillero con acciones permitidas
                if (esPlanillero && permitirAcciones && !esInhabilitado && onJugadorClick) {
                    e.stopPropagation();
                    onJugadorClick();
                }
            }}
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {entroDespues && (
                        <ArrowRight className="w-3.5 h-3.5 text-[var(--color-primary)] flex-shrink-0" />
                    )}
                    {/* Flecha si salió */}
            ...
            {esPlanillero && permitirAcciones && (
                <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                    {/* Botón cambio (solo si está en cancha y hay suplentes disponibles) */}
                    {enCancha && jugador.dorsal && onSolicitarCambio && (
                        <button
                            ...
                            disabled={estaCargando || esInhabilitado || !haySuplentesDisponibles}
                            title={haySuplentesDisponibles ? "Realizar cambio" : "No hay suplentes disponibles"}
                        >
